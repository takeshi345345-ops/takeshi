...
# --- 3. 變數初始化 (防止 NameError) ---
# 這是修正關鍵：先定義好變數，避免後面讀不到
portfolio = [] 
selected_sectors = []
LINE_TOKEN = None
...
with st.sidebar:
    ...
    # 自動監控開關
    # 判斷時間：如果是盤後，預設關閉自動監控(省資源)；盤中預設開啟
    auto_refresh_default = True
    now_check = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=8)
    if not (9 <= now_check.hour < 13 or (now_check.hour == 13 and now_check.minute <= 30)):
        auto_refresh_default = False

    auto_refresh = st.toggle("啟動自動監控", value=auto_refresh_default)
...
def scan_market(user_port):
    ...
    st.toast(f"🐕 全市場掃描中 ({len(targets)} 檔)... 請稍候")
...
if not df_show.empty:
    if portfolio:
        st.markdown("### 💼 我的庫存")
        # 修正：檢查 df_show 是否包含 'is_inv' 欄位，防止舊緩存干擾
